...
//  ─────────────────────────────────────────────────────────────────
//  RESEARCH SOURCES:
//   [1] CarPro iOS App — HDJailbreakDetection (original)
//   [2] Bajaj Allianz Insurance Wallet — AppDelegate jailbreak block
//   [3] Harsh Dwivedi — Additional checks & consolidation (2026)
//  ─────────────────────────────────────────────────────────────────
//  CHECKS INCLUDED:
//   ✓ Jailbreak path existence (70+ paths)       [1][2][3]
//   ✓ File open via fopen                         [2]
//   ✓ File stat/access                            [2]
//   ✓ Sandbox write test                          [1][2]
//   ✓ /Applications writable check               [2]
//   ✓ Suspicious dylib injection (25+ names)     [2][3]
//   ✓ DYLD_INSERT_LIBRARIES env                  [2]
//   ✓ Filesystem mount flags (statvfs/statfs)    [2]
//   ✓ Shadow jailbreak bypass (ObjC runtime)     [2]
//   ✓ fork() syscall detection                   [2]
//   ✓ Cydia/Sileo/Zebra/Filza URL schemes        [1][2][3]
//   ✓ Modern tools: unc0ver, checkra1n, palera1n [3]
//   ✓ Dopamine jailbreak paths                   [3]
//   ✓ Simulator bypass toggle (runtime)          [3]
//  ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────
#pragma mark - Simulator Bypass Toggle
// ─────────────────────────────────────────
...
/// Internal gate — returns YES if we should skip all checks (simulator is allowed through)
...
+ (NSArray<NSString *> *)allJailbreakPaths {
    return @[
        ...
        @"/Applications/Icy.app", @"/Applications/IntelliScreenx.app",
        ...
        @"/jb/offsets.plist", @"/jb/jailbreakd.plist",
        @"/jb/lzma", @"/.cydia_no_stash",

        // ── Dopamine ──────────────────────────────────────
        @"/var/jb/.installed_dopamine",
        @"/var/jb/usr/lib/libjailbreak.dylib",
        @"/var/jb/usr/sbin/sshd", @"/var/jb/etc/apt",
        ...
        @"/private/var/stash", @"/private/var/Users/",
...
// ─────────────────────────────────────────
#pragma mark - ★ PRIMARY: enforceSecurityIn:
// ─────────────────────────────────────────
...
+ (BOOL)enforceSecurityIn:(UIWindow *)window {
    ...
    if ([self isRunningOnSimulator]) {
        if (_simulatorBypassEnabled) {
            JSLog(@"Simulator detected — bypass enabled, skipping checks.");
            return NO;
        }
        JSLog(@"Simulator detected — bypass disabled, showing block.");
        [HDSecurityBlockWindow
            showWithReason:HDBlockReasonSimulator
            message:@"You are running this app on an iOS Simulator. This app is designed for real devices only and cannot be used in a simulator environment."
    ...
    if ([self isBeingDebugged]) {
        ...
        [HDSecurityBlockWindow
            showWithReason:HDBlockReasonDebugger
            message:@"A debugger has been detected. This app cannot run while being actively debugged for security reasons."
            triggerCode:@"debugger"];
        return YES;
    }

    // ── 3. Comprehensive jailbreak check ─
    ...
    if (result.isJailbroken) {
        ...
        [HDSecurityBlockWindow
            showWithReason:HDBlockReasonJailbreak
            message:@"This app has detected that your device has been jailbroken or modified. For security and privacy reasons, the app cannot run on a compromised device."
...
// ─────────────────────────────────────────
#pragma mark - Comprehensive Check with Result
// ─────────────────────────────────────────
...
+ (BOOL)canFork {
    ...
    // ⚠️ Remove for App Store builds — fork() causes rejection
    pid_t pid = fork();
    if (pid >= 0) {
...
+ (NSString *)jailbreakDetectionDetails {
    ...
    if ([self hasSuspiciousDylibsLoaded])     [reasons addObject:@"Suspicious dylib injected"];
    if ([self hasDYLDEnvironmentInjection])   [reasons addObject:@"DYLD_INSERT_LIBRARIES set"];
    if ([self hasFilesystemMountedWritable])  [reasons addObject:@"Root filesystem read-write"];
    if ([self hasShadowJailbreakBypass])      [reasons addObject:@"Shadow bypass active"];

    return reasons.count > 0
        ? [NSString stringWithFormat:@"Jailbreak detected: %@", [reasons componentsJoinedByString:@", "]]
        : @"Jailbreak detected (reason unknown).";
}
